...
int main(int argc, char *argv[])
{
	...
	static bool bIsGKeyPressed = false;
			...
			switch(event.type)
			{
				...
				case KeyPress:	
						...
						switch(keySym)
						{
							...
							case XK_G:
							case XK_g:
								if (bIsGKeyPressed == false)
								{
									LightingModelToggle = true;
									bIsGKeyPressed = true;
								}
								else
								{
									LightingModelToggle = false;
									bIsGKeyPressed = false;
								}
								break;
							default:
								break;
...
gpXVisualInfo->depth, InputOutput, gpXVisualInfo->visual, styleMask, &winAttribs);
	...
	XStoreName(gpDisplay, gWindow, "Per Fragment - Per Vertex Toggle");
