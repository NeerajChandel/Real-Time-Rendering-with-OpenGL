...
void initialize(void)
{
	void uninitialize(void);
	void resize(int,int);
	int LoadStoneTexture(GLuint*);
	int LoadKundaliTexture(GLuint*);
	...
	LoadStoneTexture(&gTextureStone);
	LoadKundaliTexture(&gTextureKundali);
...
int LoadStoneTexture(GLuint* texture)
{

	int width, height;
	unsigned char* imageData = NULL;

	imageData = SOIL_load_image("Stone.bmp", &width, &height, 0, SOIL_LOAD_RGB);
...
int LoadKundaliTexture(GLuint* texture)
{

	int width, height;
	unsigned char* imageData = NULL;

	imageData = SOIL_load_image("Kundali.bmp", &width, &height, 0, SOIL_LOAD_RGB);
